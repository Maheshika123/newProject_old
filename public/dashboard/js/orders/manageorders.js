...
function getOrder(or_id){
	...
	$("#itemTable").empty();
	result = "";
	$.post(getbaseURL()+"ajaxcontroller/getorder", {task: "getorder",or_id:or_id}, function(resultText){
        ...
        if(resultText!="error"){
            ...
            */
            if(result.length>0){
                result = result[0];
                fillInputs(result.or_id,result.or_date,result.or_duedate,result.or_contact,result.or_address,result.or_ispaid,result.or_iscomplete,result.or_method);
                showEdit();
                getItems(or_id);
            }
...
function getItems(or_id){
	$("#itemTable").empty();
	$.post(getbaseURL()+"ajaxcontroller/getorderitems", {task: "getorderitems",or_id:or_id}, function(resultText){
        console.log(resultText);
...
function fillInputs(or_id=0,date="",duedate="",contact="",address="",ispaid=0,iscomplete=0,paytype="None"){
	$("#or_id").val(or_id);
	$("#date").val(date);
	$("#duedate").val(duedate);
	$("#contact").val(contact);
...
function deleteOrder(or_id){
	$.post(getbaseURL()+"ajaxcontroller/deleteorder", {task: "deleteorder",or_id:or_id}, function(resultText){
        console.log(resultText);
        if(resultText!="error"){
        	alert("Deleted!")
